...
export default function DetailSection({ course, enrollCourse,userEnrolledCourse }) {
  const imageUrl = course?.banner?.url;
    ...
    <View style={{ padding: 10, borderRadius: 15, backgroundColor: Colors.WHITE }}>
      ...
      <View style={{ padding: 10 }}>
        ...
        <View>
          ...
          <Text style={{ fontFamily: 'outfit', color: Colors.GRAY, lineHeight: 18 }}>{course.description?.markdown}</Text>
        </View>
        <View style={{ display: 'flex', flexDirection: 'row', gap: 20, justifyContent: 'space-evenly' }}>
       {userEnrolledCourse?.length>0?   <TouchableOpacity onPress={() => enrollCourse()} style={{ padding: 15, backgroundColor: Colors.PRIMARY, borderRadius: 15, }}>
          ...
          <TouchableOpacity style={{ padding: 15, backgroundColor: Colors.SECONDARY, borderRadius: 15, }}>
            <Text style={{ fontFamily: 'outfit', color: Colors.WHITE, textAlign: 'center', fontSize: 17 }}>Membership ₹49/Month </Text>
          </TouchableOpacity>
        </View>
      </View>
    </View>
